...
# V4C NEW FEATURE: BEST LAP SLICER
SLICE_BEST_LAP = True
...
if __name__ == "__main__":
    ...
    ax.set_title(f"Lap Analysis v4c (Best Lap Slicer) - Start @ {LAP_START_TIME_SEC}s", color='white')
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Speed (km/h)")
    ax.legend(loc='lower right')
    ax.grid(True, alpha=0.2)
    
    plot_path = os.path.join(OUTPUT_DIR, "lap_analysis_v4c.png")
    ...
    if SLICE_BEST_LAP and not lap_stats.empty:
        ...
        best_duration = best_lap_row['Duration']
        
        # Calculate Slice Times with Buffer
        t_start = max(0, best_lap_row['Start Time'] - SLICE_BUFFER_SEC)
        t_end = min(VIDEO_DURATION_SEC, best_lap_row['End Time'] + SLICE_BUFFER_SEC)
        
        slice_duration = t_end - t_start
        output_filename = f"Best_Lap_{best_lap_num}_{best_duration:.2f}s.mp4"
        output_path = os.path.join(OUTPUT_DIR, output_filename)
        
        print(f"\n--- BEST LAP DETECTED: Lap {best_lap_num} ({best_duration:.2f}s) ---")
        print(f"Slicing from {t_start:.2f}s to {t_end:.2f}s (Buffer: {SLICE_BUFFER_SEC}s)...")
        
        # FFmpeg command: -ss before -i for fast seek
        # Note: -ss before -i is 'seek to pos', keyframe accurate. For perfect cut without re-encode,
        # it might snap to keyframe. But user insisted on 'no re-encoding'.
        cmd = [
            "ffmpeg", "-y",
            "-ss", f"{t_start}",
            "-i", VIDEO_PATH,
        ...
        try:
            ...
            print(f"SUCCESS! Sliced Video Saved: {output_path}")
        except Exception as e:
    ...
    for i, row in lap_stats.iterrows():
        ...
        marker = "🔥 BEST LAP" if (SLICE_BEST_LAP and int(row['Lap']) == best_lap_num) else ""
