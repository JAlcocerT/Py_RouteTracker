...
# --- 1. CONFIGURATION ---
FILE_MAIN = "/home/jalcocert/Desktop/Py_RouteTracker/Z_GoPro/output-kart25dec-1c.txt" 
VIDEO_PATH = "/home/jalcocert/Desktop/Py_RouteTracker/Z_GoPro/GX030410.MP4" # The video source
...
def parse_telemetry_with_laps(file_path):
        ...
        try:
            ...
            for line in lines:
                ...
                if m:
                    ...
                    data.append({
                        'speed_kmh': speed, # Assuming input is km/h based on previous v3b (multipier 1.0)
    ...
    # Pre-calculate Lap Times for display
    # We want a column 'current_lap' and 'last_lap_time'
    df['lap_number'] = 0
    df['last_lap_time'] = 0.0
    
    current_lap = 0
    # First segment before start line is Lap 0 (Out Lap?)
    # Logic: Lap 1 starts at first crossing?
    # Let's say: Before first crossing = Lap 0.
    
    # We can just iterate intervals
    prev_idx = 0
    for i, idx in enumerate(lap_indices):
        current_lap = i + 1
        # Set lap number for this segment
        df.loc[prev_idx:idx, 'lap_number'] = current_lap
        
        # Determine last lap time (if applicable)
        if i > 0:
            t_prev = df.iloc[lap_indices[i-1]]['time']
            t_curr = df.iloc[idx]['time']
            duration = t_curr - t_prev
            # This "last lap time" is valid for the NEXT segment
            # So from idx onwards...
            df.loc[idx:, 'last_lap_time'] = duration
...
# --- 3. HUD RENDER (V4) ---
def render_racing_hud_v4(df_data, lap_indices):
    ...
    # Force Black Background for Keying
    ...
    for spine in ax_graph.spines.values(): 
        spine.set_edgecolor('white'); spine.set_path_effects(outline)
    ...
    def update(frame):
        if frame < len(df_data):
            ...
            if last_time > 0:
                last_lap_text.set_text(f"LAST: {last_time:.2f}s")
            else:
                last_lap_text.set_text("") # Hide if no valid lap yet

            # 3. Update Map
            map_tracker.set_data([row['lon']], [row['lat']])
            trail = max(0, frame - 60)
    ...
    save_path = os.path.join(OUTPUT_DIR, "racing_hud_v4.mp4")
...
if __name__ == "__main__":
    ...
    if not df.empty:
        ...
        OUTPUT_FILE = render_racing_hud_v4(df, laps)
        print(f"\nSUCCESS! HUD v4 created: {OUTPUT_FILE}")
    else:
